...
export function createResident(newResident:Resident):Resident| undefined {
residents.push(newResident)
return newResident
}

export function updateResident(id:number, updateData:Partial<Resident>):Resident|undefined{
    for(const resident of residents){
        if(resident.id===id){
